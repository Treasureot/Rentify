...
function SignUp() {
    ...
    const createUser = async (e: React.FormEvent) => {
        ...
        try {
            ...
            } else if (response.errors && typeof response.errors === 'object' && !Array.isArray(response.errors)) {

                setFieldErrors(response.errors as Record<string, string[]>);
                setError(response.title || 'One or more validation errors occurred.');
    ...
    const flatFieldErrors = Object.entries(fieldErrors)
        .map(([field, messages]) => `${field}: ${messages.join(', ')}`)
        .join(' | ');

 
    const loginPath = role === 'Tenant' ? '/login' : role === 'Landlord' ? '/login' : '/login';
        ...
        <div className="register_content">
            ...
            <div className="login_content_right">
                <form onSubmit={createUser}>
                    ...
                    {error && (
                        <p style={{ color: 'red', fontSize: '14px', marginBottom: '4px' }}>
                            {error}
                        </p>
                    )}
